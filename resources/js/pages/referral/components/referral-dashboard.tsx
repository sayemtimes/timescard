...
export default function ReferralDashboard({ userType, stats, referralLink, recentReferredUsers }: ReferralDashboardProps) {
      ...
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{window?.appSettings?.currencySettings?.currencySymbol || "$"}{stats.totalCommissionPaid}</div>
            </CardContent>
          </Card>
                ...
                {stats.topCompanies?.slice(0, 5).map((company: any, index: number) => (
                  <div key={company.id} className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-sm text-muted-foreground">{window?.appSettings?.currencySettings?.currencySymbol || "$"}{company.total_earned}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="text-2xl font-bold">
                    {window?.appSettings?.currencySettings?.currencySymbol || "$"}{Object.values(stats.monthlyPayouts || {}).reduce((a: any, b: any) => a + b, 0)}
                  </div>
                  <p className="text-sm text-muted-foreground">{t('This year')}</p>
                </div>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="mt-2 text-2xl font-bold">{window.appSettings.formatCurrency(stats.availableBalance)}</h3>
              </div>
