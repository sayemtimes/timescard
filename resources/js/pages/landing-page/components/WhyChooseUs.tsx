      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 items-center">
          ...
          <div className={`transition-all duration-700 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'}`}>
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 bg-[#055353] text-white text-center">
              {sectionData.title || t('Why Choose vCard?')}
            </h2>
            <p className="text-lg text-gray-600 mb-8 leading-relaxed font-medium text-center">
              ...
              {reasons.map((reason, index) => {
                ...
                return (
                  <div key={index} className="flex items-start gap-4">
                    <div className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 bg-[#ff8901] ">
          ...
          <div className={`rounded-xl p-8 border border-[#ff8901] bg-[#055353] transition-all duration-700 delay-300 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}>
            <div className="text-center mb-8">
              <h3 className="text-2xl font-bold text-[#ff8901] mb-2">
            ...
            <div className="grid grid-cols-2 gap-4 sm:gap-6">
              {stats.map((stat, index) => (
                <div key={index} className="text-center p-6 bg-white rounded-lg border border-[#ff8901]">
                  <div className="text-3xl font-bold text-[#055353] mb-2">{stat.value}</div>
                  <div className="text-[#055353] font-medium">{stat.label}</div>
                </div>
              ))}
            </div>
