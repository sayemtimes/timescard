...
export default function CustomPagesIndex() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingPage) {
      put(route('directory.custom-pages.update', editingPage.id), {
        onSuccess: () => {
          setEditingPage(null);
          reset();
      ...
      post(route('directory.custom-pages.store'), {
        onSuccess: () => {
  ...
  const handleDelete = (page: CustomPage) => {
    if (confirm('Are you sure you want to delete this page?')) {
      router.delete(route('directory.custom-pages.destroy', page.id), {
        onSuccess: () => {
  ...
  const checkSlugAvailability = async (slug: string, excludeId?: number) => {
    ...
    const timeout = setTimeout(async () => {
      try {
        const response = await axios.post(route('directory.custom-pages.check-slug'), {
          slug: slug.trim(),
  ...
  const generateSlugFromTitle = async (excludeId?: number) => {
    ...
    try {
      const response = await axios.post(route('directory.custom-pages.generate-slug'), {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    router.get(route('directory.custom-pages.index'), params, { preserveState: true, preserveScroll: true });
  };
  ...
  return (
    <PageTemplate 
      title="Directory Custom Pages" 
      url="/directory/custom-pages"
      actions={[
        {
          label: 'Add Page',
            ...
            <div>
              ...
              <div className="mt-1">
                <p className="text-sm text-gray-500">
                  URL: {window.location.origin}/directory/page/{data.slug || 'your-slug'}
                </p>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="mt-1">
                <p className="text-sm text-gray-500">
                  URL: {window.location.origin}/directory/page/{data.slug || 'auto-generated-from-title'}
                </p>
                {slugValidation.loading && <p className="text-sm text-blue-600">Checking availability...</p>}
                {slugValidation.available === true && <p className="text-sm text-green-600">✓ Slug is available</p>}
                {slugValidation.available === false && <p className="text-sm text-red-600">✗ Slug is already taken</p>}
