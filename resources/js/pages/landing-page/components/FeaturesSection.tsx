...
export default function FeaturesSection({ settings, sectionData, brandColor = '#3b82f6' }: FeaturesSectionProps) {
  ...
  return (
    <section id="features" className="py-12 sm:py-16 lg:py-20 features-section" ref={ref}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`text-center mb-8 sm:mb-12 lg:mb-16  duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <h2 className="text-3xl md:text-4xl font-bold text-white bg-[#055353] p-4 rounded-lg">
            {sectionData.title || 'Powerful Features for Modern Networking'}
          </h2>
          <p className="text-lg leading-relaxed font-medium bg-[#ff8901] text-white">
        ...
        <div className={`grid grid-cols-1 ${columns >= 2 ? 'sm:grid-cols-2' : ''} ${columns >= 3 ? 'lg:grid-cols-3' : ''} ${columns >= 4 ? 'xl:grid-cols-4' : ''} gap-6 sm:gap-8 transition-all duration-700 delay-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          {features.map((feature, index) => {
            ...
            return (
              <div
                key={index}
                className="feature-card p-8 rounded-xl hover:shadow-lg transition-all duration-200"
                role="article"
                aria-labelledby={`feature-${index}-title`}
              >
                <div className="w-12 h-12 rounded-lg flex items-center justify-center mb-6 feature-card-icon" role="img" aria-label={`${feature.title} icon`}>
                  <IconComponent className="w-6 h-6" aria-hidden="true" />
                </div>
                <h3 className="text-xl font-bold feature-card-title mb-4" id={`feature-${index}-title`}>
                  {feature.title}
                </h3>
                <p className="feature-card-description leading-relaxed">
                  {feature.description}
                </p>
              </div>
            );
