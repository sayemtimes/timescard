...
import { Mail, Phone, Globe, MapPin, Calendar, Download, UserPlus, Briefcase, Award, TrendingUp, Video, Play, Youtube, QrCode } from 'lucide-react';
...
export default function ConsultingTemplate({ data, template }: ConsultingTemplateProps) {
    ...
    <div className="relative rounded-t-2xl overflow-hidden" style={{ background: `linear-gradient(135deg, ${colors.primary}, ${colors.secondary})` }}>
      <div className="absolute inset-0 opacity-10">
        <svg width="100%" height="100%" viewBox="0 0 100 100" className="fill-current text-white">
          <pattern id="consulting-grid" x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse">
    ...
    return (
      <div className="px-6 py-4" style={{ borderBottom: `1px solid ${colors.accent}` }}>
        ...
        <div className="grid grid-cols-1 gap-3">
          {aboutData.expertise && (
            <div>
              <p className="text-xs font-medium mb-2" style={{ color: colors.primary, fontFamily: font }}>{t('Areas of Expertise')}:</p>
          ...
          <div className="flex justify-between items-center">
            ...
            {aboutData.certifications && (
              <div className="text-center">
                <p className="text-xs font-medium mb-1" style={{ color: colors.primary, fontFamily: font }}>{t('Certifications')}:</p>
                <p className="text-xs" style={{ color: colors.text, fontFamily: font }}>{aboutData.certifications}</p>
              </div>
            )}
          </div>
        </div>
          ...
          {services.map((service: any, index: number) => (
            <div key={index} className="p-3 rounded-lg border-l-4" style={{ 
              ...
              <div className="flex justify-between items-center text-xs">
                {service.duration && <span style={{ color: colors.primary, fontFamily: font }}>{t('Duration')}: {service.duration}</span>}
                {service.price_range && <span style={{ color: colors.primary, fontFamily: font }}>{service.price_range}</span>}
              </div>
            </div>
          ))}
        </div>
  ...
  const renderCaseStudiesSection = (caseStudiesData: any) => {
    ...
    return (
      <div className="px-6 py-4" style={{ borderBottom: `1px solid ${colors.accent}` }}>
        ...
        <div className="space-y-3">
          {studies.slice(0, 2).map((study: any, index: number) => (
            <div key={index} className="p-3 rounded-lg" style={{ backgroundColor: colors.cardBg }}>
              <h4 className="font-medium text-sm mb-1" style={{ color: colors.primary, fontFamily: font }}>{study.title}</h4>
              {study.client_industry && <p className="text-xs mb-2 font-medium" style={{ color: colors.text, fontFamily: font }}>{t("Industry:")} {study.client_industry}</p>}
              {study.challenge && (
                <div className="mb-2">
                  <p className="text-xs font-medium" style={{ color: colors.primary, fontFamily: font }}>{t('Challenge')}:</p>
                  <p className="text-xs" style={{ color: colors.text, fontFamily: font }}>{study.challenge}</p>
                </div>
              )}
              {study.solution && (
                <div className="mb-2">
                  <p className="text-xs font-medium" style={{ color: colors.primary, fontFamily: font }}>{t('Solution')}:</p>
                  <p className="text-xs" style={{ color: colors.text, fontFamily: font }}>{study.solution}</p>
                </div>
              )}
              {study.results && (
                <div>
                  <p className="text-xs font-medium" style={{ color: colors.primary, fontFamily: font }}>{t('Results')}:</p>
                  <p className="text-xs" style={{ color: colors.text, fontFamily: font }}>{study.results}</p>
                </div>
              )}
            </div>
          ))}
      ...
      <div className="px-6 py-4" style={{ borderBottom: `1px solid ${colors.accent}` }}>
        ...
        <div className="space-y-3">
          {videoContent.map((video: any) => (
            <div key={video.key} className="rounded-lg overflow-hidden" style={{ backgroundColor: colors.cardBg }}>
              <div className="relative">
                {video.embed_url && video.embed_url.includes('<iframe') ? (
                  ...
                  <div className="relative w-full h-32">
                    <img 
                      src={video.thumbnail} 
                      alt={video.title || 'Consulting video'} 
                  ...
                  <div className="w-full h-32 flex items-center justify-center" style={{ backgroundColor: colors.primary + '20' }}>
                    <div className="text-center">
                      ...
                      <span className="text-xs" style={{ color: colors.primary, fontFamily: font }}>Consulting Video</span>
                    </div>
                  </div>
                )}
              </div>
  ...
  const renderYouTubeSection = (youtubeData: any) => {
    ...
    return (
      <div className="px-6 py-4" style={{ borderBottom: `1px solid ${colors.accent}` }}>
        ...
        <div className="rounded-lg p-4" style={{ backgroundColor: colors.cardBg }}>
          ...
          <div className="mb-4 rounded-lg overflow-hidden" style={{ backgroundColor: colors.cardBg, border: `1px solid ${colors.borderColor}` }}>
            ...
            <div className="flex-1">
              <h4 className="font-medium text-sm" style={{ color: colors.text, fontFamily: font }}>
                {youtubeData.channel_name || 'Consulting Channel'}
              </h4>
  ...
  const renderSocialSection = (socialData: any) => {
    ...
    return (
      <div className="px-6 py-4" style={{ borderBottom: `1px solid ${colors.accent}` }}>
        <h3 className="font-semibold text-sm mb-3" style={{ color: colors.primary, fontFamily: font }}>{t('Professional Networks')}</h3>
  ...
  const renderBusinessHoursSection = (hoursData: any) => {
    ...
    return (
      <div className="px-6 py-4" style={{ borderBottom: `1px solid ${colors.accent}` }}>
        <h3 className="font-semibold text-sm mb-3" style={{ color: colors.primary, fontFamily: font }}>{t('Office Hours')}</h3>
        <div className="space-y-2">
  ...
  const renderTestimonialsSection = (testimonialsData: any) => {
    ...
    return (
      <div className="px-6 py-4" style={{ borderBottom: `1px solid ${colors.accent}` }}>
        <h3 className="font-semibold text-sm mb-3" style={{ color: colors.primary, fontFamily: font }}>{t('Client Testimonials')}</h3>
                ...
                <div key={index} className="w-full flex-shrink-0 px-1">
                  <div className="p-3 rounded-lg" style={{ backgroundColor: colors.cardBg }}>
                    ...
                    <div>
                      ...
                      {review.client_title && <p className="text-xs" style={{ color: colors.text, fontFamily: font }}>{review.client_title}</p>}
                    </div>
                  </div>
                </div>
              ))}
  ...
  const renderAppointmentsSection = (appointmentsData: any) => {
    return (
      <div className="px-6 py-4" style={{ borderBottom: `1px solid ${colors.accent}` }}>
        <h3 className="font-semibold text-sm mb-3" style={{ color: colors.primary, fontFamily: font }}>{t('Schedule Consultation')}</h3>
        <div className="space-y-2">
          <Button 
            size="sm" 
            className="w-full" 
          ...
          {appointmentsData?.consultation_fee && (
            <p className="text-xs text-center" style={{ color: colors.text, fontFamily: font }}>
              {appointmentsData.consultation_fee}
            </p>
          )}
        </div>
